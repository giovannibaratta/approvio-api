...
export const validateResourceResolveRequest = (body: unknown): Either<ValidationError, ResourceResolveRequest> => {
  ...
  if (!hasOwnProperty(body, "resources")) {
    return left("missing_resources")
  }
  ...
  for (const resource of resources) {
    if (!isObject(resource)) {
      return left("invalid_resource_item")
    }

    if (!hasOwnProperty(resource, "type")) {
      return left("missing_resource_type")
    }

    if (!isNonEmptyString(resource.type)) {
      return left("invalid_resource_type_value")
    }
    ...
    if (!hasOwnProperty(resource, "id")) {
      return left("missing_resource_id")
    }

    if (!isNonEmptyString(resource.id) || !isValidUUID(resource.id)) {
      return left("invalid_resource_id_format")
    }
